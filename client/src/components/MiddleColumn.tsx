...
export default function MiddleColumn({ user, events, posts, loading }: MiddleColumnProps) {
    ...
    <>
          ...
          <div key={post.id} className="bg-white rounded-lg shadow-md overflow-hidden mb-4">
            ...
            <div className="p-3">
              ...
              {post.pictures.length > 0 && (
                <div className="mb-4">
                  {post.pictures.length === 1 && post.pictures[0].picture_url ? (
                    <div 
                      className="relative cursor-pointer hover:opacity-90 transition-opacity"
                      onClick={() => openPostModal(post, 0)}
                    >
                      <Image
                        src={post.pictures[0].picture_url}
                        alt={post.title}
                        width={0}
                        height={0}
                    ...
                    <div className="grid grid-cols-2 gap-0.5">
                      ...
                      <div className="grid grid-rows-2 gap-0.5">
                        {post.pictures.slice(1, 3).map((picture, index) => (
                          picture.picture_url && (
                            <div 
                              key={picture.id} 
                              className="relative cursor-pointer hover:opacity-90 transition-opacity overflow-hidden"
                              onClick={() => openPostModal(post, index + 1)}
                            >
                              <Image
                                src={picture.picture_url}
                    ...
                    <div className="grid grid-cols-2 gap-0.5">
                      {post.pictures.slice(0, 4).map((picture, index) => (
                        picture.picture_url && (
                          <div 
                            key={picture.id} 
                            className="relative cursor-pointer hover:opacity-90 transition-opacity overflow-hidden"
                          ...
                          >
                            <Image
                              ...
                              unoptimized
                            />
                            {index === 3 && post.pictures.length > 4 && (
                              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                <span className="text-black font-semibold text-lg drop-shadow-lg">+{post.pictures.length - 4}</span>
                              </div>
                            )}
                          </div>
                        )
      ...
      {selectedPost && (
        <ImageModal
          ...
          eventDescription={selectedPost.description}
          eventDate={selectedPost.created_at}
        />
      )}
      </div>
    </>
